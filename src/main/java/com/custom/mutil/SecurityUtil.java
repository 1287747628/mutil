...
public class SecurityUtil {
    ...
    /**
     * 创建一个Token由(“用户名”：“最后登录时间”：“数字签名”),三个部分组成
     *
     * @param userName
     * @param password
     * @param lastLoginTime
    ...
    public static String createToken(String userName, String password, long lastLoginTime) {
        StringBuilder tokenBu = new StringBuilder();
        tokenBu.append(userName);
        tokenBu.append(":");
        tokenBu.append(lastLoginTime);
        tokenBu.append(":");
        tokenBu.append(computeSignature(userName, lastLoginTime, password));
        return tokenBu.toString();
    }
    ...
    private static String computeSignature(String userName, long lastLoginTime, String password) {
        StringBuilder signBu = new StringBuilder();
        signBu.append(userName);
        signBu.append(":");
        signBu.append(lastLoginTime);
        signBu.append(":");
        signBu.append(password);
        signBu.append(":");
        signBu.append(SecurityUtil.MAGIC_KEY);
        byte[] byteMD5 = digest.get().digest(signBu.toString().getBytes(Charset.forName("UTF-8")));
     ...
     * @param signatureFromToken
     * @param password
     * @return
     */
    public static boolean validateSignature(String userNameFromToken, long lastLoginTimeFromToken,
                                            String signatureFromToken, String password) {
        if (null == signatureFromToken) {
            return false;
        }
        return signatureFromToken.equals(computeSignature(userNameFromToken, lastLoginTimeFromToken, password));
    }
